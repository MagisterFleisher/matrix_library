 ...
 */
matrix_double_t* 
initializeMatrix_double(const int i, const int j) {
    matrix_double_t *m = malloc(sizeof(matrix_double_t));

    m->i = i;
    m->j = j;

    /* Allocating an array the size of the matrix */
    m->array = calloc(i * j, sizeof(double));
...
/**
 * @brief The matrix_double_t type has two arrays that need to be cleaned up before freeing the memory for the entire struct.
 * @param m The matrix that will be freed.
 */
void
freeMatrix_double(matrix_double_t *m) {
    free(m->array);
 ...
 * @param m matrix_double_t. The matrix struct
 ...
 * @param m matrix_double_t.  The matrix struct holding the array and all metadata
 */
void
printMatrix_double(matrix_double_t *m) {
    for(size_t row = 0; row < m->i; row++) {
        for(size_t column = 0; column < m->j; column++) {
            size_t array_member = (row * m->j) + column;
            (void) printf("%lf ", m->array[array_member]);
        }
        (void) printf("\n");
    }
 ...
 */
matrix_double_t*
generateIdentityMatrix_double(const int dim) {
    assert(dim > 0);
    matrix_double_t *identity_matrix = initializeMatrix_double(dim, dim);
 ...
 * @param m matrix_double_t The matrix to copy
 * @return matrix_double_t matrix in the heap
 */
matrix_double_t*
createCopy_double(matrix_double_t *m) {
    assert(m != NULL);
    matrix_double_t *m2 = initializeMatrix_double(m->i, m->j);
    memcpy(m2->array, m->array, (m->i * m->j) * sizeof(double));
 ...
 */
matrix_double_t*
generateRandomMatrix_double(const int i, const int j, const int lower_bound, const int upper_bound) { 
    matrix_double_t *m = initializeMatrix_double(i, j);
    srand(time(0));
 ...
 * @param m matrix_double_t The matrix from which the row will be selected
...
inline
void
m_ScalarAdd_double(matrix_double_t *m, const double scalar) {
    assert(NULL != m);
 ...
 */
matrix_double_t*
m_MatrixAdd_double(matrix_double_t *m1, matrix_double_t *m2) {
    assert((m1->i == m2->i) && (m1->j == m2->j));
    matrix_double_t *m = initializeMatrix_double(m1->i, m2->j);
 ...
 */
matrix_double_t*
m_MatrixSubtract_double(matrix_double_t *m1, matrix_double_t *m2) {
    assert((m1->i == m2->i) && (m1->j == m2->j));
    matrix_double_t *m = initializeMatrix_double(m1->i, m2->j);
    for(size_t index = 0; index < (m1->i * m1->j); index++) {
        m->array[index] = m1->array[index] - m2->array[index];
...
inline
bool
m_isEqual_double(matrix_double_t *m1, matrix_double_t *m2) {
 ...
 */
float
m_eigenValue_double(matrix_double_t *m) {
    if(m_isIdentity_double(m)) {
        return 1.0;
    }
    return 0;
}
 ...
 */
complex*
m_eigenVector_double(matrix_double_t *m);

/*************************** MATRIX CHARACTERIZATIONS ************************** */

/**
 * @brief Find whether all values in the matrix are binary (i.e. 0 or 1)
 * @param m Pointer to the matrix_double_t struct.  It includes the number of rows, the number of columns, and the array of values required by the function
 * @return boolean.  True if the matrix only includes binary values.  False if the matrix includes non binary values.
 */
bool
m_isBinary_double(matrix_double_t *m) {
...
inline 
bool
m_isRow_double(matrix_double_t *m) {
...
inline 
bool
m_isSquare_double(matrix_double_t *m) {
...
inline 
bool
m_isSingleton_double(matrix_double_t *m) {
 ...
 */
bool
m_isUpperTriangular_double(matrix_double_t *m) {
    if(false == m_isSquare_double(m)) {
        return false;
    }
    ...
    for(size_t row_index = 1; row_index < m->i; row_index++) {
        for(size_t column_index = 0; column_index < row_index; column_index++) {
            if(0 != m_at_double(m, row_index, column_index)) {
                return false;
            }
        }
    }
 ...
 */
bool
m_isLowerTriangular_double(matrix_double_t *m) {
    if(false == m_isSquare_double(m)) {
        return false;
    }
    size_t i_index = 0;
    ...
    while((i_index < m->i) && (j_index < m->j)) {
        if(i_index < j_index) {
            j_index++;
            if(0 != m_at_double(m, i_index, j_index)) {
                return false;
            }
        } else {
            i_index++;
 ...
 * @param m Pointer to matrix_double_t struct object
 * @return boolean.  True if it is diagonal, false otherwise.
 */
bool
m_isDiagonal_double(matrix_double_t *m) {
    /**
 ...
 */
bool
m_isIdentity_double(matrix_double_t *m) {
    if(!m_isBinary_double(m)) {
        return false;
    }
    /**
     * To find the place which should have a 1 value, 
 ...
 */
bool
m_isNull_double(matrix_double_t *m) {
    for(int index = 0; index < (m->i * m->j); index++) {
        if(0 != m->array[index]) {
            return false;
        }
 ...
 */
bool
m_isSymmetric_double(matrix_double_t *m) {
    assert(m_isSquare_double(m));
    return true;
}
 ...
 * @param m Pointer to matrix_double_t object.
 * @param m_transpose Pointer to matrix_double_t object.
 * @return boolean.  True if orthogonal, false otherwise.
 */
bool
m_isOrthogonal_double(matrix_double_t *m, matrix_double_t *m_transpose) {
    return true;
}
 ...
 * @param m Pointer to matrix_double_t object.
 * @return boolean.  True if singular, false otherwise.
 */
bool
m_isSingular_double(matrix_double_t *m) {
    return true;
}
 ...
 * @param m Pointer to matrix_double_t object.
 * @return boolean.  True if Idempotent, false otherwise.
 */
bool
m_isIdempotent_double(matrix_double_t *m) {
    if(true == m_isIdentity_double(m)) {
        return true;
    }
    return false;
}
 ...
 * @param m Pointer to matrix_double_t object.
 * @return boolean.  True if Involutory, false otherwise.
 */
bool
m_isInvolutory_double(matrix_double_t *m) {
    return true;
}
 ...
 * @param m Pointer to matrix_double_t object.
 * @return boolean.  True if nilpotent, false otherwise.
 */
bool
m_isNilpotent_double(matrix_double_t *m) {
    return true;
}